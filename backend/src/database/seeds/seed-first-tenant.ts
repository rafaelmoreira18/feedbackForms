/**
 * Seed: creates the first hospital tenant and seeds all form templates.
 * Single database architecture — all data lives in DB_DATABASE.
 *
 * Run: npx ts-node -r tsconfig-paths/register src/database/seeds/seed-first-tenant.ts
 */
    ...
    blocks: [
      {
        ...
        questions: [
          { questionKey: 'q1', text: 'Como você avalia as condições gerais das instalações da unidade de Hemodiálise?', scale: 'rating4' as const, subReasons: ['Espaço físico percebido como inadequado ou apertado', 'Instalações com aparência de falta de manutenção', 'Ausência de estrutura adequada para longas sessões de tratamento'] as [string, string, string] },
          { questionKey: 'q2', text: 'Como você avalia a limpeza e higienização do ambiente e das poltronas/leitos de Hemodiálise?', scale: 'rating4' as const, subReasons: ['Poltronas ou leitos com higienização insatisfatória entre pacientes', 'Limpeza do ambiente realizada com pouca frequência', 'Presença de odores desagradáveis na unidade'] as [string, string, string] },
          { questionKey: 'q3', text: 'Como você avalia as condições de ventilação, iluminação e temperatura, equipamentos da unidade de Hemodiálise?', scale: 'rating4' as const, subReasons: ['Temperatura inadequada durante as sessões de hemodiálise', 'Ventilação insuficiente no ambiente da unidade', 'Equipamentos com aparência de desgaste ou funcionamento irregular'] as [string, string, string] },
        ],
      },
      {
        title: 'Bloco 2 – Experiência do Paciente (Atendimento e cuidado assistencial na Hemodiálise)',
        questions: [
          { questionKey: 'q4', text: 'Como você avalia o atendimento prestado pela equipe de enfermagem durante as sessões de Hemodiálise?', scale: 'rating4' as const, subReasons: ['Demora no atendimento durante as sessões', 'Comunicação insuficiente sobre o andamento do tratamento', 'Falta de atenção individualizada às necessidades do paciente'] as [string, string, string] },
          { questionKey: 'q5', text: 'Como você avalia o acompanhamento médico recebido durante o tratamento de Hemodiálise?', scale: 'rating4' as const, subReasons: ['Visitas médicas muito esporádicas ou superficiais', 'Pouca explicação sobre os resultados e evolução do tratamento', 'Dificuldade de acesso ao médico responsável entre sessões'] as [string, string, string] },
          { questionKey: 'q6', text: 'Como você avalia o cuidado e clareza das orientações recebidos de forma geral durante o seu tratamento de Hemodiálise nesta unidade?', scale: 'rating4' as const, subReasons: ['Orientações sobre dieta e restrições foram pouco claras', 'Dúvidas sobre o tratamento não foram respondidas de forma satisfatória', 'Falta de orientação sobre intercorrências ou situações de urgência'] as [string, string, string] },
        ],
      },
      {
        title: 'Recomendação',
...
async function seed() {
  ...
  const adminEmail = process.env.SEED_ADMIN_EMAIL || 'admin@hgm.com';
  ...
  for (const tmpl of FORM_TEMPLATES) {
    const existing = await templateRepo.findOne({
      where: { tenantId: tenant.id, slug: tmpl.slug },
    });
    if (existing) {
